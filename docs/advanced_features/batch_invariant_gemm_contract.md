...
`SGLANG_BATCH_INVARIANT_OPS=all` routes matmul, log-softmax, mean and RMSNorm through kernels whose
...
Kill switches, if you need to isolate a suspect layer:

- `SGLANG_BI_GEMM_CONFIG_TABLE=0` — ignore the table and run the pinned baseline config for every
  shape. Bits are identical either way; this only gives up the tuning.
- `SGLANG_BATCH_INVARIANT_OPS_ENABLE_MM_DEEPGEMM=0` — take the Triton kernel instead of the
  DeepGEMM route. The two are bitwise equal on the shapes we ship; the switch exists for bisecting.
