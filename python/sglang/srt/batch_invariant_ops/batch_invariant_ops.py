...
from sglang.srt.batch_invariant_ops.bi_gemm_tiera import (
...
def bi_router_topk_weights(
    ...
    if norm_topk_prob:
        if _ROUTER_RENORM_FUSED_ENABLED:
            ...
            # unchanged.
            from sglang.srt.batch_invariant_ops.router_renorm_fused import (
                fused_router_renorm,
            )
...
def get_batch_invariant_ops() -> tuple[str, ...]:
    return tuple(sorted(_batch_invariant_OPS))


def is_batch_invariant_op_enabled(op: str) -> bool:
    return _BATCH_INVARIANT_ALIASES.get(op, op) in _batch_invariant_OPS
