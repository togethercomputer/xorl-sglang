"""Exact GDN decode contract, serving side: partial-chunk-rescan decode.
...
terms). Under ``the exact GDN decode=1`` decode instead RESCANS the current
...
Requires the exact GDN prefill=1 (the scan composition this path rescans
...
class BIGDNDecodeCache:
    ...
    def prepare_step_metadata(
        ...
        static_bs1: bool | None = None,
        graph_capture: bool = False,
    ) -> BIGDNDecodeStepMetadata:
        if static_bs1 is None:
            static_bs1 = BI_GDN_BS1_STATIC
        ...
        if seq_lens.shape != slot_indices.shape:
            raise RuntimeError(
                "the exact GDN decode sequence lengths and slots differ: "
                f"{tuple(seq_lens.shape)} != {tuple(slot_indices.shape)}."
            )
    ...
    def seed_from_extend(
        ...
        if prefix_len % CHUNK_SIZE != 0:
            raise RuntimeError(
                f"the exact GDN decode requires {CHUNK_SIZE}-aligned extend prefixes; got {prefix_len}."
            )
        ...
        if suffix == 0:
            ...
            if bnd_in_pass == 0:
                ...
                # fp32 chunk-boundary chaining is exact (byte-certified)
