...
class TestFusedAddRMSNormBatchInvariant(CustomTestCase):
    """The fused residual-add + RMSNorm kernel must be bit-identical to
    RMSNorm.forward_native's RL residual path under the config it dispatches
    for (cast_x_before_out_mul=True, fp32_residual=False, fp32 weight-mul)."""

    def _run_case(self, n_rows, hidden_size, dtype, seed):
        from sglang.srt.batch_invariant_ops.batch_invariant_ops import (
        ...
        norm = layernorm_mod.RMSNorm(
            hidden_size, eps=eps, cast_x_before_out_mul=True
        ).to(dtype)
        norm.weight.data.uniform_(0.5, 1.5)
        x = torch.randn(n_rows, hidden_size, dtype=dtype)
        residual = torch.randn(n_rows, hidden_size, dtype=dtype)

        saved = layernorm_mod._OPGAP_RMSNORM_FP32_WEIGHT_MUL
        layernorm_mod._OPGAP_RMSNORM_FP32_WEIGHT_MUL = True
        try:
            with set_batch_invariant_mode(True):
                ref_out, ref_residual = norm.forward_native(x.clone(), residual.clone())
                fused_out, fused_residual = fused_add_rms_norm_batch_invariant(
                    x.clone(), residual.clone(), norm.weight.data, eps
                )
        finally:
            layernorm_mod._OPGAP_RMSNORM_FP32_WEIGHT_MUL = saved

        self.assertTrue(
            torch.equal(ref_out, fused_out),
            f"output not bit-identical (rows={n_rows}, h={hidden_size}, {dtype})",
        )
        self.assertTrue(
            torch.equal(ref_residual, fused_residual),
            f"residual not bit-identical (rows={n_rows}, h={hidden_size}, {dtype})",
        )

    def test_bit_identical_to_forward_native(self):
        for n_rows, hidden_size in [(1, 2048), (7, 2048), (64, 2048), (3, 4096)]:
            for dtype in [torch.bfloat16, torch.float16]:
                with self.subTest(n_rows=n_rows, hidden_size=hidden_size, dtype=dtype):
                    self._run_case(n_rows, hidden_size, dtype, seed=1234)
