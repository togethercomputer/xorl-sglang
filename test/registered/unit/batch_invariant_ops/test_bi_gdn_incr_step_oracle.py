...
The class of bug this gate exists to catch (gdn-incr-hunt, 2026-08-12): the
...
from sglang.kernels.ops.attention.fla.bi_gdn_prefill import solve_tril
...
def test_step_program_matches_rescan_oracle(hv, hg, defer_writeback):
    ...
    qkv_dim = 2 * hg * K_DIM + hv * V_DIM
    gen = torch.Generator(device=device).manual_seed(20260812)
...
@pytest.mark.parametrize(
    "p", (0, 5, 15, 16, 17, 31, 32, 40, 47, 48, 55, 63)
)
def test_solve_row_matches_full_solve_across_launch_configs(p, num_warps):
    device = torch.device("cuda")
    h = 16
    T = p + 1
    gen = torch.Generator(device=device).manual_seed(20260812 + p)
