...
def test_legacy_position_oracle_matches_mixed_scheduler_lengths() -> None:
    cache = _cache()
    slots = [3, 0, 2, 1]
    slot_indices = torch.tensor(slots, dtype=torch.int32)
    cache._legacy_suffix_len[:] = [7, 63, 0, 31]
    seq_lens = torch.tensor([32, 8, 1, 64], dtype=torch.int32)

    metadata = cache.prepare_step_metadata(slots, slot_indices, seq_lens)
    cache._verify_and_advance_legacy_positions(metadata)

    assert cache._legacy_suffix_len == [8, 0, 1, 32]
