...
from sglang.xorl.fla import qwen35_gdn_exact as exact
...
def test_qwen35_moe_is_rejected_as_unported():
    """This dev-based branch does not carry the canonical MoE contributor
    fold; the resolver must fail closed instead of selecting a different
    numerical program (main resolves the full MoE contract here)."""
    hf_config = _qwen_config(moe=True)
    args = _server_args(tp_size=8, dp_size=8, ep_size=8)
...
def test_qwen35_rmsnorm_v2_is_the_exact_lane_default(moe):
    ...
    if moe:
        ...
        # Family selection is stamped before the fail-closed MoE rejection.
...
def test_qwen35_dense_rejects_incompatible_arithmetic_mechanisms(name, value, message):
    hf_config = _qwen_config(moe=False)
    args = _server_args(tp_size=1, dp_size=1, ep_size=1)
    setattr(args, name, value)
...
def test_qwen35_private_resolver_installs_one_tuple_once():
    ...
    import sglang.xorl.batch_invariant as xorl_family
    import sglang.xorl.bi.bi_gemm_configs as gemm_configs
    import sglang.xorl.bi.bi_gemm_tiera as tiera
    import sglang.xorl.fla.bi_gdn_decode as decode
    import sglang.xorl.fla.bi_gdn_decode_fast as fast
    import sglang.xorl.fla.bi_gdn_decode_incr as incremental
    import sglang.xorl.fla.bi_gdn_incr_lazy_heal as heal
    import sglang.xorl.fla.bi_gdn_prefill as prefill
    ...
    with ExitStack() as stack:
        ...
        force_table.assert_called_once_with(False)
        ...
        assert "moe_fold=none" in rendered
...
def test_qwen35_sampler_contract_rescore_uses_temperature_and_fast_head():
    ...
    with (
        ...
        patch(
            "sglang.xorl.bi.bi_head_fastpath."
