...
class _ReqToTokenPoolStub:
    ...
    def translate_mamba_indices(self, mamba_indices: torch.Tensor) -> torch.Tensor:
        # Identity translate: this stub models the non-unified pool.
        return mamba_indices


class TestHybridLinearAttnMetadata(unittest.TestCase):
    def test_dp_sync_padding_uses_pad_mamba_slot(self):
        ...
        self.assertEqual(
            ...
            [23, 31, PAD_SLOT_ID, PAD_SLOT_ID],
        )
        self.assertEqual(metadata.query_start_loc.tolist(), [0, 1, 2, 3, 4])

    def test_cuda_graph_padding_poisons_padded_rows(self):
        # v0.5.17 poisons padded replay rows to -1 (mamba kernels skip them);
        # main's base remapped them to the reserved slot 0 instead.
        backend = MambaAttnBackendBase.__new__(MambaAttnBackendBase)
        backend.device = torch.device("cpu")
        backend.native_mtp_verify_steps = None
        backend.replayssm_write_pos_list = None
        backend._fused_state_indices_ok = False
        backend.req_to_token_pool = _ReqToTokenPoolStub(
            torch.tensor([17, 23, 31], dtype=torch.int32)
        )
        ...
        self.assertEqual(metadata.mamba_cache_indices.tolist(), [23, 31, -1, -1])
