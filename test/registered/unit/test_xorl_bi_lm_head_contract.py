...
from sglang.xorl.bi import bi_families_v2
...
def _paired_family(family):
    with patch("sglang.xorl.batch_invariant._FORCED_FAMILY", family):
        yield
...
class TestXorlBatchInvariantTarget(unittest.TestCase):
    ...
    def test_whole_family_plan_is_once_and_declares_peer_requirement(self):
        ...
        with (
            _paired_family("v2"),
            patch(
                "sglang.xorl.batch_invariant._CONTRACT_PLAN_LOGGED",
                False,
            ),
        ):
            kwargs = dict(
    ...
    def test_official_glm52_norm_site_map_and_qk_envelope(self):
        ...
        # main additionally pins that models/deepseek_v2.py wires every norm
        # site through this map; the GLM-5.2 model wiring is not ported to
        # this branch, so that source scan is dropped here.
...
class TestXorlBatchInvariantHeadAndSampler(unittest.TestCase):
        ...
        for family in ("v1", "v2"):
            for n in (1, 8):
                with self.subTest(family=family, n=n), _paired_family(family):
                    ...
                    with (
                        patch(
                            "sglang.xorl.batch_invariant." "bi_lm_head_full_logits",
                            return_value=expected,
                        ) as v1_head,
                        patch(
                            "sglang.xorl.batch_invariant."
    ...
    def test_bi_head_composes_literal_rank_one_lora_after_exact_base(self):
        ...
        with (
            _paired_family("v2"),
            patch(
                "sglang.xorl.batch_invariant.head_v2_full_logits_with_lse",
    ...
    def test_mixed_temperature_is_applied_once_before_sampling_and_scoring(self):
        ...
        with (
            _paired_family("v2"),
            patch(
                "sglang.xorl.batch_invariant." "head_v2_selected_logprob_from_logits",
                side_effect=score,
            ),
        ):
            xorl_bi_sample_and_score(
